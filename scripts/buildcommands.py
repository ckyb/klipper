...
class HandleCommandGeneration:
    ...
    def update_data_dictionary(self, data):
        ...
        data['commands'] = { msg: msgtag for msg, msgtag in list(msg_to_tag.items())
                             if msgtag in command_tags }
        data['responses'] = { msg: msgtag for msg, msgtag in list(msg_to_tag.items())
...
def main():
    ...
    # Parse request file
    ctr_dispatch = { k: v for h in Handlers for k, v in list(h.ctr_dispatch.items()) }
    f = open(incmdfile, 'r')
    data = f.read()
    f.close()
